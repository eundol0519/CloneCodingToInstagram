...
const apis = {
  ...
  postWrite: postInfo => instance.post(`/api/posts`, postInfo), // 게시글 작성

  //게시물 상세 페이지 (모달창)
  getDetailPost: postId => instance.get(`/api/posts/${postId}`), // 게시글 상세페이지 조회
  postLikeCancel: postId => instance.post(`/api/posts/${postId}/like`), // 좋아요 / 취소
  deletePost: postId => instance.delete(`/api/posts/${postId}`), // 게시글 삭제
